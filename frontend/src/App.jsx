...
import { Upload, X, Send, BarChart2, Activity, Calendar, Clock, User, Table as TableIcon, Search, ChevronLeft, ChevronRight, Filter, Eye, ListTree, Link as LinkIcon, Info, CheckCircle, Circle, Loader2, AlertTriangle } from 'lucide-react'
        ...
        <div className="flex-1 space-y-3 overflow-y-auto pr-1">
          {versions.map((v, i) => (
            <div 
              ...
              className={`p-3 rounded-xl border cursor-pointer transition-all ${selectedVersionId === v.id ? 'bg-blue-50 border-blue-200 outline-2 outline-blue-500/20' : 'bg-white border-gray-100 hover:border-blue-100 hover:bg-gray-50'}`}
            >
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-[9px] font-bold text-gray-400 font-mono">{v.data_date}</span>
              </div>
                ...
                {viewerTable === 'TASK' && (
                  <div className="flex gap-2">
                    <button 
                      ...
                      className={`px-3 py-2.5 rounded-xl text-xs font-bold border transition-all flex items-center gap-2 ${showCriticalOnly ? 'bg-red-50 border-red-200 text-red-700' : 'bg-white border-gray-200 text-gray-600 hover:bg-gray-50'}`}
                    >
              ...
              <div className="px-8 mt-4 flex flex-col gap-4">
                <div className="flex flex-wrap gap-4 items-center">
                  <div className="px-4 py-2 bg-white rounded-xl border border-gray-200 shadow-sm flex items-center gap-3">
                    <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Project Delay:</span>
                  ...
                  <div className="flex gap-6 items-center text-[10px] font-bold uppercase tracking-wider text-gray-400">
                    ...
                    <span className="flex items-center gap-1.5"><Circle size={12} className="text-gray-400" /> {tableData.projectAnalysis.healthMetrics?.notStartedTasks} Not Started</span>
                  </div>
                </div>
              </div>
            )}
                  ...
                  <table className="min-w-[1200px] w-full text-left border-collapse table-fixed">
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {tableData.records.length > 0 ? tableData.records
                        .filter(row => !showCriticalOnly || row.is_critical)
                        .map((row, i) => {
                          ...
                          if (status === 'COMPLETED') {
                            ...
                            statusConfig = { label: 'Delayed', color: 'bg-red-100 text-red-700 border border-red-200', icon: <AlertTriangle size={12} /> };
                          }

                          return (
                            <tr key={i} className={`hover:bg-blue-50/40 transition-colors group ${isDelayed ? 'bg-red-50/40' : isCritical ? 'bg-red-50/20' : ''}`}>
                              {(viewerTable === 'TASK' 
                                ? ['task_code', 'task_name', 'status', 'target_start_date', 'target_end_date', 'act_start_date', 'act_end_date', 'total_float_hr_cnt']
                                : viewerTable === 'WBS'
                                ? ['wbs_short_name', 'wbs_name', 'parent_wbs_id', 'seq_num', 'est_wt']
                        ...
                        <tr>
                          <td colSpan="100" className="px-6 py-32 text-center">
                            <div className="flex flex-col items-center gap-3 text-gray-400">
                              ...
                              <button onClick={() => {setTableSearch(''); setShowCriticalOnly(false);}} className="text-xs text-blue-600 font-bold hover:underline">Reset filters</button>
                            </div>
                          </td>
                        </tr>
                      )}
